...
for m in range(M * d**(4 * l)):
    ...
    for i in range(l-1, -1, -1):
        sites[0][2 * i], sites[0][2 * i + 1] = toricCode.horizontalPair(sites[0][2 * i], sites[0][2 * i + 1], cleanRight=True)
        sites[1][2 * i], sites[1][2 * i + 1] = toricCode.horizontalPair(sites[1][2 * i], sites[1][2 * i + 1], cleanRight=True)
        if i > 0:
            sites[0][2 * i - 1], sites[0][2 * i] = toricCode.horizontalPair(sites[0][2 * i - 1], sites[0][2 * i], cleanRight=True)
            sites[1][2 * i - 1], sites[1][2 * i] = toricCode.horizontalPair(sites[1][2 * i - 1], sites[1][2 * i], cleanRight=True)
